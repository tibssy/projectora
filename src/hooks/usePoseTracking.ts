import { useState, useEffect, useRef, useCallback, useReducer, useTransition } from 'react';
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case 'MODEL_LOADING': return { ...state, status: 'LOADING_MODEL' };
    case 'MODEL_READY': return { ...state, status: 'READY' };
    case 'WEBCAM_STARTED': return { ...state, status: 'RUNNING', error: null };
    case 'WEBCAM_STOPPED': return { ...state, status: 'READY' };
  ...
  useEffect(() => {
    async function createPoseLandmarker() {
      ...
      try {
        ...
        dispatch({ type: 'ERROR', payload: "Failed to load the AI model." });
      }
    }
